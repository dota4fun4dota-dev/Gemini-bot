...
# ==================== ТВОИ НАСТРОЙКИ ====================
# ВНИМАТЕЛЬНО проверь этот токен. Если ты обновлял его в BotFather, убедись, что это самый ПОСЛЕДНИЙ токен
...
async def handle_ai_request(message: Message):
    ...
    try:
        ...
        logging.error("Ошибка при запросе к ИИ")
        await status_message.edit_text("❌ Произошла ошибка при обращении к ИИ. Попробуйте позже.")

# === ЗАПУСК БОТА ===
async def main():
    # Сначала строго создаем базу данных
    init_db()
    # Затем очищаем зависшие запросы Телеграма
    await bot.delete_webhook(drop_pending_updates=True)
    # И только после этого включаем приём сообщений
