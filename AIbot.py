...
# Принудительная очистка webhook при старте для устранения конфликтов
@dp.startup()
async def on_startup():
    await bot.delete_webhook(drop_pending_updates=True)

# Функция для разбивки длинных ответов
...
async def handle_text(message: Message):
    if message.text.lower().startswith("нарисуй"):
        ...
        headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}
        # Используем формат сообщения, который хорошо переваривает API Gemini
